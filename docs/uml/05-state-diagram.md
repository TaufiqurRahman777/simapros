...
stateDiagram-v2
    ...
    pending --> approved : Prioritas Critical/High (jalur cepat)
    pending --> deprioritized : Prioritas Medium/Low
    pending --> rejected : Admin reject
    pending --> revision : Admin request revision

    revision --> pending : User submit ulang

    deprioritized --> approved : Pengaju terima jadwal baru
    deprioritized --> withdrawn : Pengaju tarik pengajuan
    ...
    note right of deprioritized
        Gerbang prioritas: nilai di bawah
        ambang jalur cepat. Pengaju melihat
        nilai prioritas, alasan HTO, dan
        usulan jadwal baru, lalu memilih
        terima atau tarik.
    end note
...
### Gerbang Prioritas (Priority Gate)

Nilai prioritas dihitung dari matriks Urgensi × Impact (`src/lib/priorityMatrix.ts`) dan menentukan percabangan setelah evaluasi:

| Hasil Matriks | Jalur | Konsekuensi |
|---------------|-------|-------------|
| `critical`, `high` | Jalur cepat | Langsung `approved`, masuk dashboard Eksekutor. `is_priority = true` |
| `medium`, `low` | Konfirmasi pengaju | Menjadi `deprioritized`. `is_priority = false` |

Ambang batas diatur oleh konstanta `FAST_TRACK_PRIORITIES` di `src/lib/priorityMatrix.ts`.

Transisi dari `deprioritized` dijalankan lewat RPC `respond_to_priority(_project_id, _decision)` — fungsi `SECURITY DEFINER` yang hanya mengizinkan pemilik proyek mengubah status dari state ini. Saat pengaju menerima, `proposed_start_date`/`proposed_end_date` disalin menjadi `start_date`/`end_date`.

### Penjelasan Status Proyek

| Status | Deskripsi | Aksi Selanjutnya |
|--------|-----------|------------------|
| `pending` | Proposal baru diajukan | Admin evaluasi: tentukan prioritas, lalu approve/deprioritize/reject/revision |
| `revision` | Perlu perbaikan | User perbaiki & submit ulang |
| `deprioritized` | Non-prioritas, menunggu keputusan pengaju | Pengaju terima jadwal baru, atau tarik pengajuan |
...
| `rejected` | Ditolak HTO | End state |
| `withdrawn` | Ditarik pengaju sendiri | End state |
