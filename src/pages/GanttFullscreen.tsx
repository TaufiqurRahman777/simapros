            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                ...
                {project.current_priority_assessment ? (
                  <>
                    <Badge variant="outline" className={cn('text-xs shrink-0', recommendationConfig[project.current_priority_assessment.calculated_recommendation].className)}>
                      {recommendationConfig[project.current_priority_assessment.calculated_recommendation].label}
                    </Badge>
                    <Badge variant="outline" className="text-xs shrink-0">
                      {project.requester_decision === 'accepted'
                        && project.current_priority_assessment.final_decision === 'deferred'
                        ? 'Jadwal diterima · Masuk portofolio'
                        : finalDecisionConfig[project.current_priority_assessment.final_decision].label}
                    </Badge>
                  </>
                ) : project.current_priority_assessment_id ? (
                  <Badge variant="outline" className="text-xs shrink-0 bg-muted text-muted-foreground border-muted-foreground/30">
                  ...
                  <Badge variant="outline" className={cn('text-xs shrink-0', priorityConfig[project.priority]?.className || 'bg-muted text-muted-foreground')}>
                    Prioritas lama: {priorityConfig[project.priority]?.label || project.priority || 'Tidak Ada'}
                  </Badge>
                )}
              </div>
